 ...
 */
	public function chargeReceipt($data) {
		...
		$dn_id = $data["dn_id"];
		$rcpt_mtID = $data["mt_id"];
		$rcpt_resp = 1;//$data["response"];
		$update_dnResult = $MtTbl->updateDN( $rcpt_mtID, $dn_id );
		
		
		//$rpt_log = "Update receipt: " . $rcpt_mtID . " | " . $dn_id . " | " . $rcpt_resp;
		$rpt_log = "hello";
		
		$Log->updateLog( "yoonic-Rcpt.log", $rpt_log );
		
		//$this->atomic_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y").'yoonic-Receipts.log', $rpt_log . PHP_EOL);
		//$this->atomic_put_contents( $_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y").'yoonic-Rcpt.log' , "Hello". PHP_EOL);
			...
			if($data["session_token"]!="") {
				...
				if($isSessionValid["ApiUser"]["result_code"]=="200") {
					...
					if ($this->save($data,false)) {
							...
							$this->atomic_put_contents( $_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y").'yoonic-Receipts.log' , $content. PHP_EOL );
					} else
						{
            				...
            				if ($affected_subscription) {
								...
								*/
								...
								$this->atomic_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y").'test.log', "charging failed". PHP_EOL);
									...
									*/
									if ($subscription_keyword == "ON TP1") { //delete the ON TP1 entry from subscription table, clear the base subscription new expiry date
										$Subscription->delete($subscription_id); 
										
										/*  New logic no need to revert back to old expiry date.
 ...
 */
	public function reminderReceipt($data)
	{
		...
		$rpt_log = "Reminder receipt: " . $rcpt_mtID . " | " . $dn_id . " | " . $rcpt_resp;
		
		$this->atomic_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y").'dn.log', $rpt_log. PHP_EOL);
		$this->atomic_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y").'yoonic-Receipts.log', $rpt_log. PHP_EOL);
		//$this->atomic_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y").'yoonic-Receipts.log', $rpt_log . PHP_EOL);
