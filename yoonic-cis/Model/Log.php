 ...
 */
	public function recordRequest($data) {
		...
		if($data) {/*
			...
			*/
			...
			$data["timestamp"] = date("Y-m-d H:i:s");//@DboSource::expression('NOW()'); //date("Y-m-d H:i:s"); use current_timestamp from mysql
	...
	public function id_generator() {
        ...
        public function updateLog( $filename, $contentlog )
        {            
            $this->atomic_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y"). $filename, $contentlog. PHP_EOL);
        }
        
        
        public function recordRaw( $data )
        {
            ...
            $this->atomic_put_contents($_SERVER['DOCUMENT_ROOT'].'/log/'.date("m_d_y").'yoonic-Raw.log', $content. PHP_EOL);
        }
